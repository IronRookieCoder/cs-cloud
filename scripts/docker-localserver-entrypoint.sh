...
#   CS_BRIDGE_AGENT_ENV (legacy alias: CS_CLOUD_AGENT_ENV) so cs-cloud's agent
#   runtime injects them into the csc subprocess.
#
# Env var naming: the preferred prefix is CS_BRIDGE_*. The CS_CLOUD_*
# equivalents still work as legacy aliases for backwards compatibility —
# every read below is "new name with old-name fallback".
set -euo pipefail

: "${CS_BRIDGE_DATA_DIR:=${CS_CLOUD_DATA_DIR:-/root/.costrict}}"
: "${CS_BRIDGE_PORT:=${CS_CLOUD_PORT:-8080}}"
: "${CS_BRIDGE_HOST:=${CS_CLOUD_HOST:-0.0.0.0}}"
: "${CS_BRIDGE_AGENT_PATH:=${CS_CLOUD_AGENT_PATH:-/usr/local/bin/csc}}"
CS_BRIDGE_AGENT_COMMAND="${CS_BRIDGE_AGENT_COMMAND:-${CS_CLOUD_AGENT_COMMAND:-}}"
CS_BRIDGE_AGENT_ENV="${CS_BRIDGE_AGENT_ENV:-${CS_CLOUD_AGENT_ENV:-}}"
...
# Propagate resolved values back under the new names so the cs-cloud binary
# (which reads CS_BRIDGE_* preferentially) sees them regardless of which
# alias the operator set.
export CS_BRIDGE_DATA_DIR CS_BRIDGE_PORT CS_BRIDGE_HOST CS_BRIDGE_AGENT_PATH
[ -n "${CS_BRIDGE_AGENT_COMMAND}" ] && export CS_BRIDGE_AGENT_COMMAND
[ -n "${CS_BRIDGE_AGENT_ENV}" ] && export CS_BRIDGE_AGENT_ENV
...
APP_DIR="${CS_BRIDGE_DATA_DIR}/cs-cloud"
mkdir -p "${APP_DIR}"
...
#   has already injected OPENAI_API_KEY/OPENAI_BASE_URL via CS_BRIDGE_AGENT_ENV.
#   Writing modelType here unblocks the /model picker for env-only providers.
# - `env` mirrors the same provider env vars we hand to CS_BRIDGE_AGENT_ENV. csc's
...
CSC_CONFIG_HOME="${COSTRICT_CONFIG_DIR:-${CS_BRIDGE_DATA_DIR}}"
...
if [ ! -f "${SETTINGS_FILE}" ]; then
  ...
  # CS_BRIDGE_AGENT_ENV. We share the same translation by computing it once here.
  settings_env_body=""
  case "${MODEL_PROVIDER}" in
    anthropic)
...
# CS_BRIDGE_AGENT_ENV. Skip if the operator already supplied CS_BRIDGE_AGENT_ENV
...
build_agent_env() {
  ...
  case "${provider}" in
    ...
    openai)
      ...
      # 覆盖 CS_BRIDGE_AGENT_ENV，此处不会再生效（因为 entrypoint 跳过翻译）。
...
if [ -z "${CS_BRIDGE_AGENT_ENV:-}" ]; then
  agent_env="$(build_agent_env "${MODEL_PROVIDER}")"
  if [ -n "${agent_env}" ]; then
    export CS_BRIDGE_AGENT_ENV="${agent_env}"
  fi
fi

echo "[entrypoint] cs-cloud localserver starting"
echo "[entrypoint]   data-dir : ${CS_BRIDGE_DATA_DIR}"
echo "[entrypoint]   bind     : ${CS_BRIDGE_HOST}:${CS_BRIDGE_PORT}"
echo "[entrypoint]   agent    : ${CS_BRIDGE_AGENT_PATH}"
echo "[entrypoint]   agent cmd: ${CS_BRIDGE_AGENT_COMMAND:-${CS_BRIDGE_AGENT_PATH} serve (default)}"
...
if [ -n "${CS_BRIDGE_AGENT_ENV:-}" ]; then
  echo "[entrypoint]   agent-env: ${CS_BRIDGE_AGENT_ENV}"
fi

exec cs-cloud _daemon \
  --data-dir "${CS_BRIDGE_DATA_DIR}" \
  --agent-path "${CS_BRIDGE_AGENT_PATH}" \
  --host "${CS_BRIDGE_HOST}" \
  --port "${CS_BRIDGE_PORT}" \
  --no-auto-upgrade
