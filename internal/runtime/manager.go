...
func (m *AgentManager) BindWorkflowSession(ctx context.Context, sessionID, cwd string, env []string, permMode string) error {
	a, ok := m.GetAgent("default")
	if !ok {
		return fmt.Errorf("no default agent available")
	}
	type sessionBinder interface {
		CreateSession(ctx context.Context, sessionID, cwd string, env []string, permMode string) error
	}
	b, ok := a.(sessionBinder)
	if !ok {
		return fmt.Errorf("default agent %q does not support session binding", a.Backend())
	}
	return b.CreateSession(ctx, sessionID, cwd, env, permMode)
}
...
func (m *AgentManager) RunWorkflowSession(ctx context.Context, sessionID, cwd, prompt string, env []string, permMode string) ([]byte, error) {
	a, ok := m.GetAgent("default")
	if !ok {
	...
	return r.RunSession(ctx, sessionID, cwd, prompt, env, permMode)
}
